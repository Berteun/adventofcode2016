...
def neighbours(grid, start, target):
    ...
    while stack:
        ...
        if len(nbs) > 2 or cur == target:
            result[last_intersection].add((cur, steps, tuple(path)))
            result[cur].add((last_intersection, steps, tuple(path)))
...
def part2(grid):
    ...
    while stack:
        ...
        nb_set = nbs[cur]
        for (nb, next_steps, h) in nb_set:
            if nb in path:
                continue
            key = (path, nb, steps)
            if key not in seen:
                seen.add(key)
                new_state = (nb, steps + next_steps, path.union(frozenset([nb])))
                stack.append(new_state)
